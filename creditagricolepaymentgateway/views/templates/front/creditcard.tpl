...
<button disabled id="stored-card" class="btn btn-primary" data-toggle="modal" data-target="#wirecard-ccvault-modal">{lFallback s='vault_use_existing_text' mod='creditagricolepaymentgateway'}</button>
<div id="new-card-text" class="invisible">{lFallback s='selected_creditcard_info' mod='creditagricolepaymentgateway'}</div>
<button id="new-card" class="invisible btn btn-primary">{lFallback s='vault_use_new_text' mod='creditagricolepaymentgateway'}</button>
{/if}
<div id="payment-processing-gateway-credit-card-form">
</div>
{if $ccvaultenabled == 'true'}
    <label for="wirecard-store-card"><input type="checkbox" id="wirecard-store-card" /> {lFallback s='vault_save_text' mod='creditagricolepaymentgateway'}</label>
{/if}